<template>
  <button class="mt-12 mx-24 bg-blue-900 relative mb-12">
    asdas
    <div class="shadows absolute bottom-0 bg-blue-900"></div>
    <div
      class="h-full buttons absolute right-0 text-left font-normal ring-4 ring-blue-900 ml-3"
    >
      <p class="text-xl text-blue-500 font-bold mb-1">
        Reprt <span class="font-normal text-base text-black ml-2">5/5/21</span>
      </p>
      <p class="text-3xl font-bold mt-4 text-blue-900">Diku's report series 06/2021</p>
      <p class="text-2xl font-semibold">
        Crisis and creativity. Consequences of the Covid-19 pandemic for international
        educational cooperation
      </p>
      <p>
        International educational cooperation has been hit hard by the corona pandemic.
      </p>
    </div>
  </button>
</template>

<script>
export default {
  name: "BulletinBottomLeft",
};
</script>

<style>
.buttons {
  width: 98%;
  height: 96%;
}
.shadows {
  width: 90%;
  height: 90%;
}
</style>
