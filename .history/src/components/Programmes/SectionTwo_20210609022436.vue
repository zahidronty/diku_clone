<template>
  <div class="pt-24 pb-24 px-24 flex flex-col bg-red-50">
    ...
    <p class="mt-12 font-semibold text-xl text-blue-900 mb-3">Topic</p>
    <div class="flex flex-wrap py-3 w-3/4">
      <p class="py-3 px-2 bg-secondary text-xl text-white mr-3 mb-4">Artistic research</p>
    </div>

    <p class="mt-12 font-semibold text-xl text-blue-900 mb-3">Sort By</p>
    <div class="flex flex-wrap py-3 w-3/4">
      <p class="py-3 px-2 bg-secondary text-xl text-white mr-3 mb-4">Application deadline
        <i class=" text-xl ml-2 fas fa-arrow-down"></i>
      </p>
    </div>

    <div class="relative mt-12 ">{{checkingSearchBox}}
      <input v-model="searchData" class="searchBox absolute z-50 px-6 text-xl m-1 text-blue-500 placeholder-blue-500 py-3 focus:outline-none" placeholder="Search"/>
      <i v-if="!isSearchBox" class="icon fas fa-search text-blue-900 text-2xl" :class="{ show: !isSearch, hide: isSearch }"></i>
      <i v-if="isSearchBox" @click="clear()" class="icon fa fa-times text-blue-900 text-2xl" aria-hidden="true" :class="{ show: !isSearch, hide: isSearch }" ></i>
    </div>
...
<script>
  ...
  export default {
    ...
    setup () {
      ...
      watchEffect(() => {
        (searchInput.value ? isSearchBox.value = true:  isSearchBox.value = false)
    ...
    computed:{
      ...
      checkingSearchBox(){
        return (this.searchData ? isSearchBox=true : isSearchBox=false)
      }
