<template>
  <div :class="style.main">
    ...
    <div :class="style.seachContainer">
      <input v-model="searchData" :class="style.searchInput" placeholder="Search"/>
      <i v-if="!searchData" :class="style.searchIcon"></i>
      <i v-if="searchData"  @click="clear()" class="style. icon fa fa-times text-blue-900 text-2xl" aria-hidden="true"></i>
    </div>
    ...
    data(){
      return{
        ...
        style:{
          ...
          sortOptionContainer: "absolute top-12 z-90 w-full bg-secondary text-xl text-white",
          sortOption: "py-3 px-2 hover:bg-blue-900",
          searchContainer: "relative mt-6",
          searchInput: "searchBox absolute z-10 px-6 text-xl text-blue-500 placeholder-blue-500 py-3 focus:outline-none",
          searchIcon: "icon fas fa-search text-blue-900 text-2xl"
        }
      } 
    },
    mounted(){
