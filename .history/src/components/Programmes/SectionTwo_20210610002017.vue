<template>
  <div class="just pt-24 pb-24 px-24 flex flex-col bg-red-50">
    ...
    <div class="flex flex-wrap text-xl text-white py-3 w-3/4">
      <div class="py-3 px-2 bg-secondary mr-3 mb-4 hover:bg-blue-900">Elementary school and kindargarten </div>
      <div class="py-3 px-2 bg-secondary mr-3 mb-4 hover:bg-blue-900">Upper secondary education </div>
      <div class="py-3 px-2 bg-secondary mr-3 mb-4 hover:bg-blue-900">Adult learning </div>
      <div class="py-3 px-2 bg-secondary mr-3 mb-4 hover:bg-blue-900">Higher vocational education </div>
      <div class="py-3 px-2 bg-secondary mr-3 mb-4 hover:bg-blue-900">Higher education </div>
    </div>

    <p class="mt-3 font-semibold text-xl text-blue-900 mb-3">Topic</p>
    <div class="flex flex-wrap py-3 w-3/4">
      <p class="py-3 px-2 bg-secondary text-xl text-white mr-3 mb-4 hover:bg-blue-900">Artistic research</p>
    </div>
    ...
    <div class="relative z-30 flex flex-col w-64">
      ...
      <div v-if="isSort" class="absolute top-12 z-90 w-full bg-secondary text-xl text-white ">
        <p @click="sortA(1)" :class="{orangeSort: sortSelect==1}"  class="py-3 px-2 hover:bg-blue-900 ">Application deadline</p>
        <p @click="sortA(2)" :class="{orangeSort: sortSelect==2}"  class="py-3 px-2 hover:bg-blue-900 ">Programe Name</p>
        <p @click="sortA(3)" :class="{orangeSort: sortSelect==3}"  class="py-3 px-2 hover:bg-blue-900 ">Action Name </p>
      </div>
    </div>
    ...
    <div class="relative mt-24">
      <div class="mb-2 mr-2 shadow-lg">
        <div v-for="programData in programmesData" :key="programData.id">
        <router-link :to="{name:'ProgramPage', params:{title: programData.id}}" >
        <List  
    ...
    methods: {
      ...
      sortA(type){
        if(type == '1'){
          ...
          this.sortSelect = 1
          let arr = this.programmesData
          dateArray.sort((a,b) => new Moment(a)
console.log(mySortedArray)

          const newarr = arr.deadline.sort((a, b) => {
            new Moment(a.deadline).format('DD-MM-YYYY') - new Moment(b).format('DD-MM-YYYY'));
          });
          console.log(newarr)
