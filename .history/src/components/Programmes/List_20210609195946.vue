<template>
 <div @mouseover="isMove=true" @mouseleave="isMove=false" class="w-full border-b-2 border-secondary z-80 px-6 py-2 flex items-center justify-between bg-white">
  <div class="w-2/3">
   ...
   <p class="text-medium"> {{subtitle}} {{ moment(todaysDate).format("ddd MMM DD, YYYY [at] HH:mm a") }}
 </p>
  </div>
  <div class="flex">
   <p class=" font-semibold text-blue-900 mr-3"> Application deadline:</p> <span> {{compare}} {{deadline}} </span>
  </div>
...
<script>
 ...
 export default {
  ...
  computed:{
   compare(){
    if(moment(this.deadline).isAfter('2020-12-31', 'year') ){
     return 0;
    }
    else{
     return 1;
