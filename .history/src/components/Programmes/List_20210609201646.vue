<template>
 <div @mouseover="isMove=true" @mouseleave="isMove=false" class="w-full border-b-2 border-secondary z-80 px-6 py-2 flex items-center justify-between bg-white">
  ...
  <div class="flex ">
   <p :class="{slash:  }" class="font-semibold text-blue-900 mr-3"> Application deadline: <span class="text-black"> {{deadline}} </span></p> 
  </div>
...
<script>
 ...
 export default {
  ...
  created: function () {
    this.moment = moment;
    (moment(this.deadline).isAfter(new Date())?over=true:over=false);
