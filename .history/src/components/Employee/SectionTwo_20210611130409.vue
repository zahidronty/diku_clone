<template>
 <div :class="style.main">
    <div class="style. pt-8 px-24">
     <p class="style. text-2xl text-blue-900 font-semibold">Here you will find an overview of employees at Diku.</p>
     <div class="style. relative mt-12 ">
      <input @blur="suggest" @click="departmentsSort" v-model="searchData" class="searchBox absolute z-50 px-6 text-xl m-1 text-blue-500 placeholder-blue-500 py-3 focus:outline-none" placeholder="Search by name, department and work area"/>
      <ul v-if="isSuggest" class="absolute top-0 pt-24 z-30 pb-3 bg-white w-full border-2 border-blue-900" >
       <li class="style. mx-6 border-b-2 border-secondary text-secondary text-lg font-semibold pl-6 " @click="suggestSelected" v-for="department in departments" :key="department">{{department}}</li>
      </ul>
      <i v-if="!searchData" class="style. icon fas fa-search text-blue-900 text-2xl"></i>
