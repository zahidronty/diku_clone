<template>
 <div class="w-screen h-full bg-red-50 py-8">
    <div class="pt-8 px-24">
     ...
     <div class="relative">
      <input @click="departmentsSort" v-model="searchData"  class="w-full my-12 px-6 text-xl text-blue-500 placeholder-blue-500 py-3 focus:outline-none" placeholder="Search by name, department and work area"/>
      <i class="icon fas fa-search text-blue-900 text-2xl" :class="{ show: !isSearch, hide: isSearch }"></i>
      <i @click = "this.searchData=''" class="icon fa fa-times text-blue-900 text-2xl" aria-hidden="true" :class="{ show: isSearch, hide: !isSearch }"></i>
     <ul class="absolute z-50 bg-white w-full border-4 border-blue-900" :class="{showSuggestion: isSuggest, hideSuggestion: !isSuggest }"  >
       <li class="mx-6 border-b-4 border-secondary text-secondary text-lg font-semibold pl-6 " @click="suggestSelected" v-for="department in departments" :key="department">{{department}}</li>
      </ul>
     </div>
   ...
   <button @click="this.userLimit += 10" class="focus:outline-none w-screen text-center text-xl text-blue-900 ">
