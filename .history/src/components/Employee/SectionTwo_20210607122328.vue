<template>
 <div class="w-screen h-full bg-red-50 py-8">
    <div class="pt-8 px-24">
     ...
     <div class="relative">
      <input @click="isSearch=true" v-model="searchData"  class="w-full my-12 px-6 text-xl text-blue-500 placeholder-blue-500 py-3 focus:outline-none" placeholder="Search by name, department and work area"/>
      <i class="icon fas fa-search text-blue-900 text-2xl" :class="{ show: isSearch, hide: !isSearch }"></i>
      <i class="fa fa-times" aria-hidden="true" :class="{ show: !isSearch, hide: isSearch }"></i>
     </div>
    </div>
