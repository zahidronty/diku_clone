   ...
   <div :class="[{expandMainLeft: isExpandLeft}, {expandMainRight: isExpandRight}, style.mainBoxContainer]">
      ...
      <div :class="style.info">
       ...
       <div @mouseover="isActive = true" @mouseleave="isActive = false" @click ="expand" :class="{ hide: isExpand, show: !isExpand }">
        <p :class="style.moreInfo" >More information
        <i :class="[{move: isActive},style.icon]"></i>
       </p> 
      </div>
     </div>
     <i @click="shrink" class="absolute right-0 mr-12 fa fa-times text-5xl text-blue-900" :class="{ show: isExpand , hide: !isExpand }"  aria-hidden="true"></i>
  ...
  data() {
   return {
    ...
    style: {
      ...
      mainBoxContainer: "flex focus:outline-none justify-start ml-1 main bg-white ring-4 ring-blue-900 p-2",
      proPic: "w-48",
      info:"h-full",
