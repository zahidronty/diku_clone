   ...
   >
     <div class="flex w-full">
      ...
      <div class="h-full px-3">
       ...
       <p class="text-lg font-bold text-blue-900 mb-2">{{position}}</p>
       <div  :class="{ show: isExpandLeft || isExpandRight , hide: !isExpandLeft || isExpandRight }" >
        <p class="text-secondary font-medium text-lg"> {{phone}} </p>
        <p class="text-secondary font-medium text-lg"> {{email}} </p>
       </div>
       <div class="text-lg inline underline text-secondary font-bold" 
        @mouseover="isActive = true"
        @mouseleave="isActive = false"
        @click ="expand"
         :class="{ hide: isExpandLeft || isExpandRight , show: !isExpandLeft || isExpandRight  }"
       >
        More information
     ...
     <div   >
      <i @click="shrink"  class="fa fa-times text-5xl text-blue-900" :class="{ show: isExpandLeft || isExpandRight , hide: !isExpandLeft || isExpandRight }"  aria-hidden="true"></i>
     </div>
   </button>
  </div>
</template>
