<template>
  <div
    @mouseover="isActive = true"
    @mouseleave="isActive = false"
    class="relative w-full h-full"
  >
    <div :class="[style.shadow,{'bottom-0':topBottom=bottom}]"/>
    <button :class="style.mainBox">
      <p :class="style.primaryText"> {{primary}} </p>
      <p :class="style.secondaryText"> {{secondary}} </p>
      <p :class="style.tertiaryText">{{tertiary}}
        <i :class="[style.icon,{ move: isActive }]"/>
      </p>
    </button>
  </div>
</template>

<script>
  export default {
    name: "BoxTopRight",
    props: ['primary', 'secondary', 'tertiary','topBottom','leftRight'],
    data() {
      return {
        isActive: false,
        topBottom:"bottom",
        leftRight:"left",
        style:{
          shadow:"shadows absolute top-0 bg-blue-900",
          mainBox:"buttons absolute right-0 bottom-0 text-left font-semibold text-3xl bg-white ring-4 ring-blue-900 ml-3 px-8",
          primaryText:"text-blue-900 font-bold mb-1",
          secondaryText:"secondaryColor font-bold",
          tertiaryText:"text-xl font-normal mt-4 inline",
          icon:"fas fa-arrow-right text-md",
        }
      };
    },
  };
</script>

<style>
  .buttons {
    width: 98%;
    height: 96%;
  }
  .shadows {
    width: 90%;
    height: 90%;
  }
  .move {
    transform: translateX(30px);
    transition-duration: 1s;
    transition-timing-function: ease-in-out;
  }
</style>
