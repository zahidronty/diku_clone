 ...
 <div class="h-screen relative pt-12 px-24 bg-white ">
    <div class="h-full flex flex-col justify-between">
      <Header class="z-50" @modal="modal"/>
        ...
        <p :class="{slash:over}" class="text-xl text-blue-900 font-semibold"> Application deadline: <span class="text-black"> {{deadline.moment().format("dddd, MMMM Do YYYY")}} </span> </p>
    </div>
  </div>  
</template>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      moment(this.deadline).format("dddd, MMMM Do YYYY, h:mm:ss a");
    },
